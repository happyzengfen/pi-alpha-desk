...
const { writeFileAtomicSync, writePrivateFileAtomicSync } = await createJiti(import.meta.url).import("./atomic-file.ts");
...
test("atomically replaces a regular file with binary contents", (t) => {
  const root = tempRoot(t);
  const destination = path.join(root, "upload.bin");
  fs.writeFileSync(destination, "old");

  writeFileAtomicSync(destination, Buffer.from([0, 1, 2, 255]));

  assert.deepEqual(fs.readFileSync(destination), Buffer.from([0, 1, 2, 255]));
  assert.deepEqual(fs.readdirSync(root), ["upload.bin"]);
});
