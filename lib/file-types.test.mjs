...
test("detects image, audio, and document preview paths", async () => {
  ...
  assert.equal(getDocumentMime("/tmp/budget.xlsx"), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  assert.equal(getDocumentMime("/tmp/legacy.doc"), "application/msword");
  ...
  assert.equal(isDocumentPreviewPath("/tmp/budget.XLS"), true);
  assert.equal(isDocumentPreviewPath("/tmp/data.csv"), true);
...
test("extracts extensions from mixed path styles", async () => {
  ...
  assert.equal(documentPreviewKind("/tmp/report.DOC"), "doc");
  assert.equal(documentPreviewKind("/tmp/table.XLSX"), "xlsx");
