...
function resolvePathForAccess(filePath: string, resolver: typeof path | typeof path.win32): string {
  const absolute = resolver.resolve(filePath);
  let existing = absolute;
  const missing: string[] = [];

  while (!existsSync(existing)) {
    const parent = resolver.dirname(existing);
    if (parent === existing) return absolute;
    missing.unshift(resolver.basename(existing));
    existing = parent;
  }

  try {
    return resolver.resolve(realpathSync.native(existing), ...missing);
  } catch {
    return absolute;
  }
}
...
export function isFilePathAllowed(target: string, allowedRoots: Set<string>): boolean {
  for (const root of allowedRoots) {
    ...
    const normalized = resolvePathForAccess(target, resolver);
    const normalizedRoot = resolvePathForAccess(root, resolver);
