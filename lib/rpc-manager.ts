...
export function createPersistedEmptyForkSession(
  sourceManager: SessionManager,
  parentSessionFile: string,
): { newSessionFile: string; newSessionId: string } {
  const newManager = SessionManager.create(sourceManager.getCwd(), sourceManager.getSessionDir());
  const newSessionFile = newManager.newSession({ parentSession: parentSessionFile });
  ...
  writeFileSync(newSessionFile, `${JSON.stringify({
    ...
    cwd: newManager.getCwd(),
    parentSession: parentSessionFile,
  })}\n`, { flag: "wx" });

  return { newSessionFile, newSessionId };
}
    ...
    switch (type) {
      ...
      case "fork": {
        ...
        if (!entry.parentId) {
          ...
          ({ newSessionFile, newSessionId } = createPersistedEmptyForkSession(sessionManager, currentSessionFile));
        } else {
          ...
          newSessionId = SessionManager.open(newSessionFile, sessionDir).getSessionId();
        }
