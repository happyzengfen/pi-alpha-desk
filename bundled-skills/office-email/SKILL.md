...
The `accounts.json` configuration file (in this skill's folder) holds one or more mail accounts; legacy `.env` is supported as a fallback. **Do NOT ask the user to create or edit these files — just run the commands directly.** If credentials are wrong, the scripts will return a clear error message; only then should you inform the user to check their email settings.

Both files live in this skill's directory (same folder as this SKILL.md file). The scripts load them automatically via absolute paths, regardless of the current working directory. To configure an account, create `accounts.json`:
  ...
  "accounts": [
    {
      ...
      "smtpPort": 465,
      "smtpSecure": true,
      "smtpFrom": "you@163.com"
    }
  ]
}
```

Use the provided scripts as the only email transport interface. Do not write temporary IMAP/SMTP scripts, do not use raw sockets, OpenSSL, `net`, `tls`, or alternate mail clients, and do not inspect `.env`, `accounts.json`, or script source unless the official command output explicitly reports missing configuration and the user asks you to diagnose it. If an official command fails or times out, report that command result and suggest checking the email configuration (accounts.json / .env); do not implement a fallback protocol client.
...
Never ask the user to send an email authorization code, app password, account password, or other credential in chat. If an account is disabled, incomplete, or has invalid credentials, ask the user to enable or update it in the skill's email configuration (accounts.json / .env), then rerun the official command.
...
依赖已随本 skill 自带（scripts/../node_modules，离线可用），无需安装。

## Security Notes

- **内网部署**：SMTP/IMAP 服务器必须内网可达（公司邮件服务器 / 内网 Exchange 等）；无法访问公网邮箱（163/Gmail 等）
- **凭据保护**：邮箱密码/授权码存于本 skill 目录的 `accounts.json` 或 `.env`（**明文，务必**）：① 加入 `.gitignore` 不入库 ② 限制文件访问权限（如 Windows 仅当前用户可读） ③ 不写入对话、不写入日志
